...
% methods_c = {'ZF', 'MMSE', 'LRA (method=ZF)', 'SZF (d=1, samples=32)', 'SZF (d=1, samples=64)', 'SZF (d=1, samples=128)', 'SZF (d=1, samples=256)', 'SZF (d=1, samples=512)', 'SZF (d=1, samples=1024)'};
% methods_c = {'ZF', 'MMSE', 'LRA (method=ZF)', 'SZF (d=0.25, samples=256)', 'SZF (d=0.5, samples=256)', 'SZF (d=1, samples=256)', 'SZF (d=2, samples=256)', 'SZF (d=4, samples=256)', 'SZF (d=8, samples=256)'};
% methods_c = {'ZF', 'MMSE', 'LRA (method=ZF)', 'SZF (d=1, samples=64)', 'SZF (d=1, samples=256)', 'SZF (d=1, samples=65536)'};
% methods_c = {'ZF', 'MMSE', 'LRA (method=ZF)', 'SZF (d=1, samples=256)'};
methods_c = {'SZF (d=1, k=-1, samples=256'};

cmap1 = colormap(lines(3));
cmap2 = colormap(gray(3));
...
%% Simulation
for i = 1:numel(methods_c)
    method = methods_c{i};
    PeBits_v = [];
    PeSymb_v = [];
    avgSimTime = [];
    detector = makeDetectorBeta(method, cons, consEnergy, modulation);
    for snr = snr_v
        SumSimTime = 0;
    ...
    if i<4
        subplot(4, 2, [1, 3, 5])
        semilogy(snrDB_v, PeBits_v, 'Marker', 'x', 'Color', cmap1(i, :))
        hold on
        subplot(4, 2, [2, 4, 6])
        semilogy(snrDB_v, PeSymb_v, 'Marker', 'x', 'Color', cmap1(i, :))
        hold on
        subplot(4, 1, 4)
        plot(snrDB_v, avgSimTime, 'Marker', 'x', 'Color', cmap1(i, :))
        hold on
    else
        subplot(4, 2, [1, 3, 5])
        semilogy(snrDB_v, PeBits_v, 'Marker', 'x', 'Color', cmap2(i-3, :))
        hold on
        subplot(4, 2, [2, 4, 6])
        semilogy(snrDB_v, PeSymb_v, 'Marker', 'x', 'Color', cmap2(i-3, :))
        hold on
        subplot(4, 1, 4)
        plot(snrDB_v, avgSimTime, 'Marker', 'x', 'Color', cmap2(i-3, :))
        hold on
    end
