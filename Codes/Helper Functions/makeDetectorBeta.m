function detector = makeDetectorBeta(method, cons, consEnergy, modulation)
    [methodName, params] = parseMethodString(method);

    switch upper(methodName)
        case 'ZF'
            detector = @(y_v,H_m,snr,N0,Nt,Nr) ZF(y_v, H_m, cons);

        case 'MMSE'
            detector = @(y_v,H_m,snr,N0,Nt,Nr) MMSE(y_v, H_m, snr, cons);

        case 'LRA-ZF'
            alpha = getOr(params,'alpha',0.75);
            detector = @(y_v,H_m,snr,N0,Nt,Nr) LRA(y_v, H_m, snr, cons, consEnergy, alpha, 'ZF', modulation);

        case 'LRA-MMSE'
            alpha = getOr(params,'alpha',0.75);
            detector = @(y_v,H_m,snr,N0,Nt,Nr) LRA(y_v, H_m, snr, cons, consEnergy, alpha, 'MMSE', modulation);

        case 'SD'
            d = getOr(params,'d',0.7);
            detector = @(y_v,H_m,snr,N0,Nt,Nr) SD(y_v, H_m, d, cons, consEnergy, modulation);

        case 'OGD'
            detector = @(y_v,H_m,snr,N0,Nt,Nr) OGD(y_v, H_m, cons, consEnergy, modulation);

        case 'DZF'
            detector = @(y_v,H_m,snr,N0,Nt,Nr) DZF(y_v, H_m, cons);

        case 'DMMSE'
            detector = @(y_v,H_m,snr,N0,Nt,Nr) DMMSE(y_v, H_m, snr, cons);

        case 'RHO'
            rho_eps = getOr(params,'eps',0.001);
            rho_iter = getOr(params,'iter',4);
            detector = @(y_v,H_m,snr,N0,Nt,Nr) RHO(y_v, H_m, cons, consEnergy, modulation, rho_eps, rho_iter);

        case 'SPARSE_METHOD'
            detector = @(y_v,H_m,snr,N0,Nt,Nr) Sparse_Method(y_v, H_m, cons);

        case 'SZF'
            detector = @(y_v,H_m,snr,N0,Nt,Nr) SZF(y_v, H_m, snr, cons, consEnergy);

        case 'SZF2'
            detector = @(y_v,H_m,snr,N0,Nt,Nr) SZF2(y_v, H_m, snr, cons, consEnergy);

        case 'SMMSE'
            detector = @(y_v,H_m,snr,N0,Nt,Nr) SMMSE(y_v, H_m, snr, cons);

        otherwise
            error('Unknown detection method: %s', methodName);
    end
end

function v = getOr(params, field, default)
    if isfield(params, field)
        v = params.(field);
    else
        v = default;
    end
end

function [name, params] = parseMethodString(s)
    params = struct();
    if isstring(s), s = char(s); end
    s = strtrim(s);
    if numel(s)>=2 && ((s(1)=='"' && s(end)=='"')||(s(1)=='''' && s(end)=='''')), s=s(2:end-1); s=strtrim(s); end

    name = regexp(s, '^[^(]+', 'match', 'once');
    argCell = regexp(s, '\((.*?)\)', 'tokens', 'once'); 
    if isempty(argCell), return; end
    argStr = argCell{1};
    parts = strsplit(argStr, ',');
    for i=1:numel(parts)
        kv = strtrim(parts{i});
        if isempty(kv), continue; end
        idx = regexp(kv, '=', 'once');
        if isempty(idx)
            key = matlab.lang.makeValidName(kv);
            params.(key) = true;
        else
            key = matlab.lang.makeValidName(strtrim(kv(1:idx-1)));
            val = strtrim(kv(idx+1:end));
            if numel(val)>=2 && ((val(1)=='"'&&val(end)=='"')||(val(1)==''''&&val(end)=='''')), val=val(2:end-1); params.(key)=val; continue; end
            vnum = str2double(val);
            if ~isnan(vnum), params.(key)=vnum; else params.(key)=val; end
        end
    end
end
